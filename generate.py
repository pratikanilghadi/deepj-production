...
class Generation():
    ...
    def step(self):
        ...
        """
        # Create variables - using torch.no_grad() instead of volatile
        with torch.no_grad():
            style = torch.tensor(self.style, dtype=torch.float32).unsqueeze(0)
        ...
        for prev_prob, evts, state in self.beam:
            with torch.no_grad():
                if len(evts) > 0:
                    prev_event = torch.tensor(one_hot(evts[-1], NUM_ACTIONS), dtype=torch.float32).unsqueeze(0)
                else:
                    prev_event = torch.zeros((1, NUM_ACTIONS), dtype=torch.float32)
            ...
            for _ in range(self.beam_size):
                # Sample action - Fixed: Added num_samples=1 parameter
                output = probs.multinomial(num_samples=1)
                event = output[0, 0].item()
                
                # Create next beam
                seq_prob = prev_prob * probs[0, event].item()
